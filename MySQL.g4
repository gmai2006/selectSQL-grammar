...
/** show statement */
show_statement
    : SHOW TABLES FROM (schema_name | '?' INTEGER_NUM)
    ;
...
select_list:
  ( displayed_column | ASTERISK) ( COMMA displayed_column )*
;
...
subquery:
  LPAREN select_statements RPAREN
;
...
alias : ( AS_SYM )? column_name ;

index_name : ID ;

partition_name : ID ;

column_name : ID | STATUS_SYM | TRIGGERS_SYM | SSL_SYM | CODE_SYM | TEXT_SYM | OPEN_SYM | COMMENT_SYM
            | LEVEL_SYM
            | ACTION
            | VALUE_SYM
            | DATA_SYM
...
bool_primary:
	  ( l_predicate relational_op r_predicate ) 
...
//custom expr just for JPA
custom_expr : '?' INTEGER_NUM ;
...
function_call:
	...
	| (  group_functions LPAREN ( ALL | DISTINCT )? bit_expr RPAREN  )
        | (  group_functions LPAREN ASTERISK  RPAREN  )
;
...
delete_multiple_table_statement1:
	table_spec (DOT ASTERISK)? (COMMA table_spec (DOT ASTERISK)?)*
	FROM table_references
	(where_clause)?
;
delete_multiple_table_statement2:
	FROM table_spec (DOT ASTERISK)? (COMMA table_spec (DOT ASTERISK)?)*
